...
export const useOfflineGains = (isLoaded: boolean, totalCPS: number, lastSaved: string | undefined, setGameState: React.Dispatch<React.SetStateAction<GameState>>, setStats: React.Dispatch<React.SetStateAction<StatsState>>) => {
  ...
  useEffect(() => {
    if (isLoaded && !offlineGainsProcessed.current) {
      const lastSavedTime = lastSaved ? Date.parse(lastSaved) : Date.now();
